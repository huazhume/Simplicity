...
@interface ESPuzzleGameStatus ()

@end

@implementation ESPuzzleGameStatus {
    id<ESGamePathSearchStatus> _parentStatus;
    NSInteger _gValue;
    NSInteger _hValue;
    NSInteger _fValue;
}
...
+ (instancetype)statusWithMatrixOrder:(NSInteger)matrixOrder image:(UIImage *)image {
    ...
    ESPuzzleGameStatus *status = [[ESPuzzleGameStatus alloc] init];
    ...
    for (NSInteger row = 0; row < matrixOrder; row ++) {
        for (NSInteger col = 0; col < matrixOrder; col ++) {
            ...
            ESPuzzleGamePiece *piece = [ESPuzzleGamePiece pieceWithID:ID ++ image:[UIImage imageWithCGImage:imgRef]];
...
- (BOOL)equalWithStatus:(ESPuzzleGameStatus *)status {
    return [self.pieceArray isEqualToArray:status.pieceArray];
}

- (void)removeAllPieces {
    [self.pieceArray enumerateObjectsUsingBlock:^(ESPuzzleGamePiece * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)moveToIndex:(NSInteger)index {
    ESPuzzleGamePiece *temp = self.pieceArray[self.emptyIndex];
...
- (void)setParentStatus:(id<ESGamePathSearchStatus>)parentStatus {
    _parentStatus = parentStatus;
}

- (id<ESGamePathSearchStatus>)parentStatus {
    return _parentStatus;
}

- (NSMutableArray<id<ESGamePathSearchStatus>> *)childStatus {
...
- (void)addChildStatusIndex:(NSInteger)index toArray:(NSMutableArray *)array {
    // 排除父状态
    if ([self parentStatus] && [(ESPuzzleGameStatus *)[self parentStatus] emptyIndex] == index) {
        return;
    }
    if (![self canMoveToIndex:index]) {
        return;
    }
    ESPuzzleGameStatus *status = [self copyStatus];
...
- (NSInteger)estimateToTargetStatus:(id<ESGamePathSearchStatus>)targetStatus {
    ESPuzzleGameStatus *target = (ESPuzzleGameStatus *)targetStatus;
    ...
    for (NSInteger index = 0; index < self.pieceArray.count; ++ index) {
        ...
        ESPuzzleGamePiece *currentPiece = self.pieceArray[index];
        ESPuzzleGamePiece *targetPiece = target.pieceArray[index];
